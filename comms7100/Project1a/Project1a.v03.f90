...
!reading the data
	...
	read(1, *) dataPoints
	do i=1, 10
		print '(i2,A,2(es26.12,2x,A,4x))', i, ":   ", dataPoints(1,i), unitsX, dataPoints(2, i), unitsY
	end do
...
!the good stuff
	...
	do while(cnt < 3)
		...
		if (flag) then
			...
			print '(A,1p,es20.10)', 'The Error is ', error
			
			!beta
			beta = makeBeta(funcs, dataPoints, N, temp, guess, M)
			print '(/,A,1p,*(es20.10,2x))', 'The beta array is: ', beta
		...
		print '(1p,*(es14.6,2x))', alphaSolve
		
		deltaGuess = guess + alphaSolve	!This line is the issue. 
		print '(/,A,1p,*(es14.6,2x))', 'old paramenters: ', guess
		print '(/,A,1p,*(es14.6,2x))', 'new paramenters: ', deltaGuess
		
		newError = findError(funcs, dataPoints, N, temp, deltaGuess, M)
		deltaError = error - newError
		print '(/,A,1p,es20.10)', 'old error: ', error
		print '(A,1p,es20.10)', 'new error: ', newError
		print '(A,1p,es20.10)', 'change in error: ', deltaError
...
!Final Statistics
	...
	print '(/,A,1p,*(es14.6,2x))', 'Final paramenters: ', guess
	...
	print '(/,A,1p,e12.4,A)', 'R-Factor: ', 100 * findRFact(funcs, dataPoints, N, temp, guess, M), '%'
