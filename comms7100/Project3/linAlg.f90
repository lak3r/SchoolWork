module linAlg
	...
	!determinant
		recursive function det(mat, N) result(determinant)
			integer, intent(in) :: N
			real(16), intent(in) :: mat(N,N)
			real(16), dimension(N-1,N-1) :: litMat
			real(16) :: determinant, holder
			integer :: i, j, k
	...
	!Transpose a matrix
		function trans(A, n, m) result(At)
			...
			real(16), intent(in) :: A(n,m)
			real(16), dimension(m,n) :: At
			integer :: i
